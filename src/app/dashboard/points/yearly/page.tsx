...
const getRankBadge = (rank: number): { color: string; icon: React.ReactNode } => {
  ...
  return { color: 'bg-purple-100 text-purple-700', icon: null };
};
...
export default function YearlyPointsPage() {
  ...
  const [yearFilThai, setYearFilThai] = useState<number>(getCurrentYear() + 543);
  ...
  const fetchSalesData = async (year?: number) => {
    try {
      ...
      let response;
      if (year) {
        response = await fetch("/api/sales/yearly", {
          method: "POST",
      ...
      if (result.success && result.data) {
        ...
        setYearFilThai(result.data.yearFilThai);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <Link href="/dashboard/points/monthly" className="hover:text-purple-600 transition-colors">
          คะแนน
        </Link>
        <span>/</span>
...
function YearlyReport({ report }: { report: YearlyReportData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="relative overflow-hidden bg-gradient-to-r from-purple-600 to-indigo-700 rounded-2xl text-white p-6"
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-purple-100 text-sm mt-1">ปี {report.year} (พ.ศ. {report.year + 543})</p>
          ...
          <div className="text-right">
            ...
            <p className="text-purple-100 text-sm mt-2">เป้าหมายทั้งปี</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          ...
          สรุปค่าคอมมิชชั่น (ปี {report.year})
        </h3>
      ...
      >
        <div className="p-4 border-b bg-gradient-to-r from-purple-50 to-indigo-50">
          <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            ...
            สรุปตามประเภทสินค้า (ปี {report.year})
          </h3>
              ...
              {report.productSummary.map((product, index) => (
                <tr 
                  ...
                  className={`
                    ...
                    ${product.code === 'Total' ? 'bg-purple-50 font-semibold' : index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}
                  `}
                >
                  ...
                  <td className="px-4 py-3 text-right text-purple-600">{product.ratioCR.toFixed(2)}%</td>
                  <td className="px-4 py-3 text-right">{formatNumber(product.PB, 2)}</td>
                  <td className="px-4 py-3 text-right text-purple-600">{product.ratioPB.toFixed(2)}%</td>
                  <td className="px-4 py-3 text-right">{formatNumber(product.CUMS, 2)}</td>
                  <td className="px-4 py-3 text-right text-purple-600">{product.ratioCUMS.toFixed(2)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function SummaryCard({ 
  ...
  value: string; 
  subLabel?: string;
  color: 'purple' | 'indigo' | 'violet' | 'fuchsia';
  highlight?: boolean;
}) {
  const colorClasses = {
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${highlight ? 'ring-2 ring-fuchsia-400 ring-offset-2' : ''}
      `}
    >
...
function RankingCard({ 
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${gradientColors[color]} opacity-10 rounded-full -translate-y-10 translate-x-10`} />
      ...
      <div className="mt-4 flex items-end justify-between">
        ...
        <div className="text-right">
          ...
          <p className={`text-lg font-bold ${textColors[color]}`}>{percentage}%</p>
        </div>
      </div>
